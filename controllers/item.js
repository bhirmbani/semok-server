...
const processStatsValue = (progress, base, stretch) => {
  ...
  if (base > stretch) {
    if (progress > base) {
      ...
      result.value =
        120 /
        (100 - (progress - stretch) * (120 - 100) / (stretch - base)) *
        100;
      result.stats = 'green';
    ...
    if (progress === base) {
      ...
      result.value =
        105 /
        (100 + (progress - stretch) * (105 - 100) / (base - stretch)) *
        100;
      result.stats = 'green';
    } else if (progress < base) {
      result.value = (1 + (progress - base) / base) * 100;
...
methods.create = (req, res, next) => {
  if (!req.headers.token) {
    ...
    if (!req.body.name) {
      res.json({
        msg: { context: 'Gagal buat item', content: 'nama item harus diisi' },
        ok: false
      });
    } else if (!req.body.freq) {
      res.json({
        msg: {
          context: 'Gagal buat item',
          content: 'frekuensi pelaporan item harus diisi'
        },
        ok: false
      });
      ...
      res.json({
        msg: {
          context: 'Gagal buat item',
          content: 'Bobot harus berupa angka dan tidak ada huruf'
        },
        ok: false
      });
    } else {
      models.Item.findOne({
        ...
        if (!foundItem) {
          models.Item.findAll({
            ...
            const sameName = allItem.filter(
              each => each.name.toLowerCase() === formattedName
            );
            if (sameName.length < 1) {
              ...
              res.json({
                ref: 222,
                msg: {
                  context: 'Apa Anda tidak Salah Input?',
                  content: `Soalnya item dengan nama ${
                    sameName[0].name
                  } sudah ada. Mungkin Anda mau edit nama itemnya?`
                },
                ok: false
              });
            }
          ...
          res.json({
            ...
            msg: {
              context: 'Gagal buat item',
              content: `item dengan nama ${name} sudah ada.`
            },
            ok: false
          });
        }
...
methods.getItemByCategoryName = (req, res, next) => {
  if (!req.headers.token) {
    ...
    models.Item.findAll({
      ...
      res.json({
        itemByCategory,
        msg: 'berhasil mendapatkan item berdasarkan kategori',
        ok: true
      });
    });
  }
...
methods.getItemByWorkerName = (req, res, next) => {
  if (!req.headers.token) {
    ...
    models.Worker.findAll({
      ...
      if (workers.length < 1) {
        res.json({
          msg: `tidak ditemukan pekerja dengan id ${req.params.workerId}`,
          ok: false
        });
      } else {
            ...
            .then(items => {
              if (items.length < 1) {
                res.json({
                  msg: `${worker.role} ${
                    worker.name
                  } tidak punya item. silakan tambah atau delegasikan`,
                  ok: false
                });
              }
              res.json({
                worker,
                items,
                msg: 'ambil data item berdasarkan pekerja berhasil',
                ok: true
              });
            });
        });
      ...
      if (duplicateDelegateItem !== null) {
        models.Worker.findOne({
          ...
          models.Item.findOne({
            ...
            res.json({
              duplicateDelegateItem,
              msg: `tidak bisa mendelegasikan item ${item.name} kepada ${
                worker.role
              } ${
                worker.name
              } karena user tersebut sudah menerima delegasi untuk item ini`,
              ok: false
            });
          });
        });
        ...
        models.Worker.findOne({
          ...
          if (decoded.role === 'asmen') {
            if (worker.role === 'manager') {
              res.json({
                msg: 'asmen tidak bisa mendelegasikan item kepada manager',
                ok: false
              });
                ...
                if (item === null) {
                  models.Worker.findOne({
                    ...
                    models.Item.findOne({
                      ...
                      res.json({
                        msg: `tidak bisa mendelegasikan item ${
                          itemRef.name
                        } kepada ${workerRef.role} ${
                          workerRef.name
                        } karena bukan milik ${decoded.role} ${decoded.name}`,
                        ok: false
                      });
                    });
                  });
                        ...
                        models.Bobot.create({
                          ...
                          models.Info.create({
                            ...
                            res.json({
                              ...
                              bobotRef,
                              itemRef,
                              msg: `berhasil mendelegasikan item ${
                                itemRef.name
                              } kepada ${worker.role} ${
                                worker.name
                              }. Didelegasikan oleh: ${decoded.name}`,
                              ok: true
                            });
                          });
                        });
                  ...
                  models.Item.findOne({
                    ...
                    if (filteredItems.length < 1) {
                      res.json({
                        msg: `gagal karena item ini belum didelegasikan kepada ${
                          decoded.role
                        } ${decoded.name}`,
                        ok: false
                      });
              ...
              res.json({
                ref: '471',
                msg: 'tidak ada fitur untuk mendelegasikan item kepada admin',
                ok: false
              });
            }
            ...
            if (worker.role === 'admin') {
              ...
              res.json({
                msg:
                  worker.role === 'manager'
                    ? 'tidak bisa mendelegasikan item dari manager ke sesama manager'
                    : 'tidak bisa mendelegasikan item dari manager ke staff',
                ok: false
              });
            } else {
              ...
              res.json({
                ref: '464',
                msg: 'tidak ada fitur pendelegasian kepada admin',
                ok: false
              });
            }
          }
          ...
          if (decoded.role === 'staff') {
            if (worker.role === 'asmen' || worker.role === 'manager') {
              ...
              models.Item.findOne({
                ...
                res.json({
                  msg: `${decoded.role} tidak bisa mendelagasikan item ${
                    itemRef.name
                  } kepada ${worker.role} ${worker.name}. ${
                    decoded.role
                  } hanya bisa mendelegasikan item kepada ${decoded.role}`,
                  ok: false
                });
              });
            } else if (worker.role === 'staff') {
...
methods.description = (req, res, next) => {
  ...
  if (!req.headers.token) {
    res.json({
      msg: 'butuh jwt token untuk membuat deskripsi item',
      ok: false
    });
    ...
    models.Item.findOne({
      ...
      item.getInfos().then(info => {
        const filteredWorker = info.filter(
        ...
        if (filteredWorker.length > 0) {
          ...
          item
            ...
            .then(desc => {
              res.json({
                desc,
                msg: `sukses mengubah deskripsi untuk item ${item.name}`,
                ok: true
              });
            });
        } else {
          ...
          res.json({
            msg: `Anda belum punya akses ke item ${
              item.name
            } karena belum didelegasikan`,
            ok: false
          });
        }
      });
...
methods.getItemById = (req, res, next) => {
  if (!req.headers.token) {
    ...
    models.Item.findAll({
      ...
      if (items.length < 1) {
        res.json({
          msg: `tidak ditemukan item untuk id ${req.params.itemId}`,
          ok: false
        });
      }
  ...
  if (!req.headers.token) {
    ...
    models.Worker.findOne({
      ...
      const filteredItem = worker.Items.filter(
        item =>
          item.WorkerItem.itemId === Number.parseInt(req.body.itemId, 10) &&
      ...
      models.Item.findOne({
        ...
        if (decoded.role === 'admin') {
          if (item === null) {
            res.json({
              ref: 881,
              msg: `tidak ditemukan item dengan id ${req.body.itemId}`,
              ok: false
            });
          } else {
            item.getTargets().then(targetRef => {
              const targets = targetRef.map(target => target);
              const filteredTargets = targets.filter(
                target => target.period === req.body.period
              );
              if (filteredTargets.length < 1) {
                res.json({
                  ref: 888,
                  msg: `tidak ditemukan period ${
                    req.body.period
                  } pada item ini`,
                  ok: false
                });
          ...
          res.json({
            ...
            msg: `${decoded.role} ${
              decoded.name
            } tidak punya akses terhadap item ini`,
            item,
            ok: false
          });
        } else if (
          filteredItem[0].createdBy === decoded.name &&
          decoded.role !== 'admin'
        ) {
          if (!req.body.base && !req.body.stretch) {
            res.json({
              ref: 906,
              msg: 'nilai base atau stretch tidak boleh kosong',
              ok: false
            });
          } else if (req.body.base === req.body.stretch) {
            res.json({
              ref: 1000,
              msg: 'nilai base dan stretch tidak boleh sama',
              ok: false
            });
          } else {
            item.getTargets().then(itemWithTarget => {
              const filteredTarget = itemWithTarget.filter(
              ...
              if (filteredTarget.length < 1) {
                ...
                filteredTarget[0]
                  ...
                  .then(updatedTarget => {
                    item.getStatuses().then(statusItem => {
                      const filterStatus = statusItem.filter(
                        each => each.period === req.body.period
                      );
                      if (filterStatus.length < 1) {
                        res.json({
                          ...
                          msg: `item ${
                            item.name
                          } berhasil diperbarui targetnya untuk bulan ${utility.processMonthName(
                            updatedTarget.period
                          )}.`,
                          msg2:
                            'belum ada status dan performance pada item dengan period ini',
                          updatedTarget,
                          item,
                          ok: true
                        });
                        ...
                        item.getProgresses().then(progress => {
                          item.getPerformances().then(performance => {
                            item.getBobots().then(bobot => {
                              item.getStatuses().then(status => {
                                if (bobot.length > 1) {
                                  ...
                                  const progressVal = Number.parseInt(
                                    filterProgress[0].value,
                                    10
                                  );
                                  const base = Number.parseInt(
                                    filteredTarget[0].base,
                                    10
                                  );
                                  const stretch = Number.parseInt(
                                    filteredTarget[0].stretch,
                                    10
                                  );
                                  const updatedStats = processStatsValue(
                                    progressVal,
                                    base,
                                    stretch
                                  );
          ...
          res.json({
            ref: 925,
            msg:
              'Anda harus izin dulu pada yang membuat item untuk mengupdate base dan stretch',
            ok: false
          });
        }
      });
            ...
            if (worker.length < 1) {
              ...
              models.Performance.findAll({
                ...
                if (perf.length > 0) {
                  ...
                  item.getStatuses().then(statuses => {
                    perf.forEach(each => {
                      statuses.forEach(stats => {
                        if (each.period === stats.period) {
                          each.update({
                            value:
                              req.body.value / 100 * (stats.value / 100) * 100
                          });
                        }
                      });
                    });
                    const filteredBobots = item.Bobots.filter(
                      each => each.WorkerId === Number(req.body.workerId, 10)
                    );
                    filteredBobots[0]
                      .update({
                      ...
                      .then(updatedBobot => {
                        res.json({
                          updatedBobot,
                          msg: `Berhasil mengubah nilai bobot milik user ${
                            worker[0].name
                          } pada item ${item.name}`,
                          ok: true
                        });
              ...
              res.json({
                msg: `Anda tidak bisa mengubah bobot milik pekerja lain. Silakan login dulu sebagai ${
                  worker[0].role
                } ${worker[0].name}`,
                ok: false
              });
    ...
    models.Item.findOne({
      ...
      if (item === null) {
        ...
        models.Unit.update(
          ...
          res.json({
            ref: '972',
            msg: `sukses mengubah nama satuan di item ${item.name}`,
        ...
        res.json({
          ref: '976',
          msg: `tidak bisa mengubah nama satuan karena item ini belum didelegasikan kepada ${
            decoded.role
          } ${decoded.name}`,
          ok: false
        });
        ...
        models.Unit.update(
          ...
          res.json({
            ref: '987',
            msg: `sukses mengubah nama satuan di item item ${item.name}`,
            updatedName: req.body.name,
            itemId: item.id,
            ok: true
          });
  ...
  if (!req.headers.token) {
    ...
    models.Item.findOne({
      ...
      if (item === null) {
        ...
        const filteredWorker = item.Workers.filter(
          each => decoded.name === each.name
        );
        if (filteredWorker.length < 1) {
          ...
          item.getTargets().then(targetRef => {
            ...
            if (filteredTargets.length < 1) {
              res.json({
                ref: 1038,
                msg: `tidak ada period ${
                  req.body.period
                } untuk item dengan id ${req.body.itemId}`,
                ok: false
              });
            } else {
              item.getProgresses().then(progressRef => {
                const filteredProgress = progressRef.filter(
                  progress => progress.period === req.body.period
                );
                if (
                  filteredTargets[0].base === null ||
                  filteredTargets[0].base === '0'
                ) {
                  res.json({
                    ref: 1157,
                    msg: `tidak bisa update progress. pastikan nilai base bulan ${utility.processMonthName(
                      filteredProgress[0].period
                    )} pada item ${item.name} diisi dulu`,
                    ok: false
                  });
                } else if (
                  filteredTargets[0].stretch === null ||
                  filteredTargets[0].stretch === '0'
                ) {
                  res.json({
                    ref: 1159,
                    msg: `tidak bisa update progress. pastikan nilai stretch bulan ${utility.processMonthName(
                      filteredProgress[0].period
                    )} pada item ${item.name} diisi dulu`,
                    ok: false
                  });
                          ...
                          .then(perf => {
                            item.getBobots().then(bots => {
                              if (bots.length > 1) {
                                    ...
                                    models.StatusItem.create({
                                      ...
                                      if (perf.length < 1) {
                                        ...
                                        res.json({
                                          ...
                                          msg: `item ${
                                            item.name
                                          } berhasil diperbarui nilai progressnya untuk bulan ${utility.processMonthName(
                                            req.body.period
                                          )}`,
                                          ok: true
                                        });
                                ...
                                // bobot hanya satu
                                const filteredBobots = bots.filter(
                                  bobot => bobot.WorkerId === decoded.id
                                );
                                const statuses = val.map(each => each);
                                const filteredStatuses = statuses.filter(
                                  each => each.period === req.body.period
...
methods.getItemDeviationInCertainPeriod = (req, res, next) => {
  ...
  if (!req.headers.token) {
    res.json({
      msg: 'butuh jwt token untuk mendapatkan deviation item ini',
      ok: false
    });
  } else {
    models.Status.findOne({
      ...
      if (!deviation) {
        res.json({
          ok: false,
          msg: `tidak ada deviation untuk item dengan id ${itemId} dan period ${period}`
        });
      }
      ...
      if (!item) {
        ...
        models.Item.findOne({
          ...
          if (!itemWithName) {
            item
              ...
              .then(updatedItem => {
                res.json({
                  ...
                  updatedItem,
                  ok: true,
                  msg: `berhasil mengubah nama item ${before} menjadi ${
                    updatedItem.name
                  }`
                });
              });
            ...
            res.json({
              msg: `sudah ada item dengan nama ${itemWithName.name}`,
              ok: false
            });
          }
        });
                ...
                models.ProgressItem.findAll({
                  ...
                  models.StatusItem.findAll({
                    ...
                    if (performanceItem.length > 0 && statusItem.length > 0) {
                      item.getPerformances().then(performances => {
                        item.getProgresses().then(progresses => {
                          item.getStatuses().then(statuses => {
                            item.getTargets().then(targets => {
                              ...
                              res.json({
                                ref: 1506,
                                msg: `item ${item.name} berhasil dihapus`,
                                ok: true
                              });
                            });
                          });
